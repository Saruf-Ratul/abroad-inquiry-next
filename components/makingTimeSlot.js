...
/**
 * Making time slot according to business logic.
 * @type {Function}
 * @param {Object} datas -The apporintment data 
 * @param {string} datas.schedule -The schedule time
 * @param {Object} datas.booked_data -The booked schedule.  
 * @param {Date} date 
...
const makingTimeSlot = (datas, date) => {
  ...
  if (datas.length > 0) {
    ...
    let current_time = manageTime.addingDate(
    ...
    if (manageTime.isTodaysDate(date)) {
      for (let i = 0; i < schedule_len; i++) {
        ...
        while (true) {
          let appointment_ending_time =
            appointment_starting_time + meeting_duration;
          let available = true;
          let time_slot_data = {
          ...
          for (let j = 0; j < booked_time_slot.length; j++) {
            if (booked_time_slot[j] == time_slot_data.start_time) {
              available = false;
            }
          }
          ...
          if (
            manageTime.conpareTime(
              manageTime.getLocalTime(time_slot_data.start_time),
              current_time
            ) &&
            manageTime.getLocalDate(time_slot_data.start_time) ==
              manageTime.getLocalDate(date) &&
            available
          ) {
